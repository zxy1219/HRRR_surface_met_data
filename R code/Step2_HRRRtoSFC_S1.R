...
site <- read.dbf("/home/xzhang/gfs/GIS/NO2/NO2_2019_site_HRRR_airport_102008.dbf", as.is = FALSE)
...
# Scenario 1 (no max or min limits, not consider SBL or CBL). 
# Major parameters:
...
for (i in 1:length(file_00)){
  ...
  #if H=0, fill H and L using previous rows.
  ...
  dat_HRRR2$CVS <- pracma::nthroot(9.81*dat_HRRR2$SFC_sensible_heat2*as.numeric(dat_HRRR2$SFC_PBLH)/(1.293*1004*dat_HRRR2$TempK),3)
  dat_HRRR2$L_day <- 0-((1.293*1004*dat_HRRR2$TempK)*(dat_HRRR2$u^3))/(0.4*9.81*dat_HRRR2$SFC_sensible_heat2)
  dat_HRRR2$L_SBL <- ((dat_HRRR2$TempK)*as.numeric(dat_HRRR2$u)^2)/(0.4*9.81*0.09*(1-0.5*as.numeric(dat_HRRR2$EATM_cloud_cover/100)^2))
  ...
  temp1 <- data.frame(Year=dat_HRRR3$year_local,#V1
                      ...
                      u=dat_HRRR3$u[1], #V7
                      w=abs(dat_HRRR3$CVS[1]), #V8
                      VPTG=-9, #V9
                      Zic=as.numeric(dat_HRRR3$SFC_PBLH[1]), #V10
                      Zim=as.numeric(dat_HRRR3$EHLT_geoheight[1]),#V11
                      L=dat_HRRR3$L_day[1], #V12
                      z0 = as.numeric(dat_HRRR3$SFC_roughness[1]),#V13
...
write.csv(hrrr1,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[1],".csv"),row.names = F)
write.csv(hrrr2,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[2],".csv"),row.names = F)
write.csv(hrrr3,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[3],".csv"),row.names = F)
write.csv(hrrr4,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[4],".csv"),row.names = F)
write.csv(hrrr5,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[5],".csv"),row.names = F)
write.csv(hrrr6,paste0("/home/xzhang/gfs/outputs/NO2_sites/Step2_raw/HRRR_2019_ID",site_grid_id[6],".csv"),row.names = F)
